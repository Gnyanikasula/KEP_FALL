"""
eval_p5.py — SHIELD Phase 5 evaluation harness.  FINAL / FROZEN.
...
from verdict import (kg_retrieve, rag_retrieve, rag_knowledge,
                     build_context, _synthesize,
                     SYSTEM_KNOWLEDGE, SYSTEM_VERDICT)
from citation_norm import canonical_from_kg, canonical_from_chunk, extract_from_prose

CQ_PATH   = Path("eval_questions_full.json")
GOLD_PATH = Path("gold_standard_full.json")
OUT_DIR   = Path("Results"); OUT_DIR.mkdir(exist_ok=True)
CKPT      = OUT_DIR / "eval_p5_checkpoint.json"
...
def report(results, arms):
    ...
    json.dump(results, open(OUT_DIR / "eval_p5_results.json", "w"), indent=2)
    print(f"\nWrote {OUT_DIR/'eval_p5_results.json'}")
